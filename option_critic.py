...
class OptionCriticConv(nn.Module):
    def __init__(self,
        ...
        self.features = nn.Sequential(
            nn.Conv2d(self.in_channels, 32, kernel_size=8, stride=4, bias=False),
            nn.ReLU(),
        ...
        # TODO: torch.zeros  or torch.randn
        # self.options_W = nn.Parameter(torch.randn(num_options, 512, num_actions))
        # self.options_b = nn.Parameter(torch.randn(num_options, num_actions))
    ...
    def predict_option_termination(self, state, current_option):
        fc = self.fc(state)
        termination = self.terminations(fc)[:, current_option].sigmoid()
    ...
    def get_terminations(self, state):
        fc = self.fc(state)
        termination = self.terminations(fc).sigmoid() 
        return termination
    ...
    def get_action(self, state, option, epsilon):
        if self.testing or self.rng.random() > epsilon:
            with torch.no_grad():
                fc = self.fc(state)
                logits = self.option_layer[option](fc)
    ...
    def get_action_(self, state, option):
        fc = self.fc(state)
        logits = self.option_layer[option](fc)
        # logits = fc @ self.options_W[option] + self.options_b[option]
